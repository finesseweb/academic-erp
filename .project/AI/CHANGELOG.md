...
- Added server-side Razorpay Orders API adapter for ACTIVE TEST credential profiles.
- Added `online_payment_transactions` provider transaction/audit table.
- Added manage-only ₹1 TEST order action on Razorpay TEST profiles.
- LIVE order creation remains blocked until TEST verification QA passes.
- Added HMAC checkout-signature verification primitive for the next payment step.
- Full Test Data Cleanup removes local online gateway transaction records while preserving credentials/routing configuration.

## ADR186 — Test Data Cleanup canonical UI restoration
- Restored the current full Test Data Cleanup UI/menu baseline after ADR185 regression.
- Added Gateway Test Orders as an additive cleanup entity only.
- Gateway credentials, profiles and Fee Head routing remain preserved.


## 2026-09-11 — ADR 184 QA PASS + ADR 187 Cashfree/PayU TEST parity
- ADR 184 Razorpay TEST QA passed with real TEST credentials: ₹1 order creation, ERP persistence, Razorpay Dashboard match, invalid-credential rejection and LIVE-profile flask suppression all confirmed.
- Added Cashfree Sandbox TEST order adapter using App ID / Secret Key, official Create Order flow and ₹1 credential-test order action.
- Added PayU TEST credential/API probe using Merchant Key / Salt and the official `verify_payment` command hash flow; no fake payment is created for this credential probe.
- Added provider-specific TEST flask actions for Razorpay, Cashfree and PayU; LIVE profiles remain excluded from TEST actions.
- Added Cashfree webhook signature verification/reconciliation foundation using `x-webhook-signature` + `x-webhook-timestamp` and the Cashfree Secret Key.
- Added PayU callback/webhook reverse-hash verification/reconciliation foundation using Merchant Key + Salt, including additional-charge and splitInfo-aware reverse hashing.
- Webhook callbacks update provider transaction state only; they do not post accounting entries. `FeePaymentService` remains the sole accounting authority for the later fee-linked checkout ADR.
- Fixed credential-test failure persistence so FAILED provider attempts are not rolled back with the outbound API exception; network calls are no longer held inside a DB transaction.
- Extended Gateway Test Orders cleanup to include PayU credential API probes as well as provider TEST orders.
- Removed NTT DATA/Atom from the supported provider registry and UI/provider selection. Existing legacy NTT rows are left untouched in the database but are no longer surfaced or accepted by the current provider registry.
- No new database migration required.

## 2026-09-11 — ADR 187 Cashfree Sandbox QA PASS
- Verified Cashfree TEST/Sandbox integration with real Sandbox credentials.
- ERP successfully created a ₹1.00 Cashfree Payment Gateway order and stored the provider transaction.
- Matching order confirmed in Cashfree Sandbox dashboard (S2S Integration).
- Cashfree webhook implementation remains code-complete but QA-deferred until the local ERP is exposed through a public HTTPS callback/tunnel.
- PayU TEST probe and provider webhook QA remain pending.

## 2026-09-11 — ADR 187 PayU TEST QA PASS
- PayU TEST Merchant Key + Salt successfully authenticated through the ERP `verify_payment` probe.
- `online_payment_transactions` persisted the TEST API probe as `CREDENTIAL_TEST_API` with API verification status.
- Retained gateway connectivity status is now: Razorpay PASS, Cashfree PASS, PayU PASS.
- Runtime webhook delivery/signature/idempotency QA remains intentionally pending until a public HTTPS callback endpoint is available.

## 2026-09-11 — ADR 188 Fee-linked TEST checkout + verified posting — IMPLEMENTED / OWNER QA REQUIRED
- Added TEST-only online fee checkout from Payment Collection & Allocation.
- Existing FeePaymentService allocation rules are previewed before provider initiation and remain the sole accounting/posting authority after provider verification.
- Fee Head routes resolve the exact ACTIVE TEST credential profile. Mixed-profile selections and unassigned Fee Heads fail closed before checkout.
- Razorpay: Standard Checkout order + server signature/payment verification before posting.
- Cashfree: Sandbox payment_session checkout + server PAID/SUCCESS verification before posting.
- PayU: Hosted TEST checkout + reverse-hash return + server verify_payment before posting.
- `online_payment_transactions` now links Fee Demand / Admission / Fee Payment and tracks verified/posting timestamps.
- Posting is idempotent through fee_payment_id. Verified provider payments that cannot be posted due to changed ERP balances are retained as PAYMENT_VERIFIED_UNPOSTED for reconciliation.
- Razorpay webhook endpoint added; Razorpay/Cashfree/PayU webhook handlers can hand verified FEE_PAYMENT events to the same posting service. Runtime webhook QA remains pending.
- LIVE checkout remains blocked.
- Fee Payment Test Data Cleanup now removes any linked online transaction before deleting the receipt/payment.


## 2026-09-11 - Documentation Synchronization

Updated project documentation against the current academic-erp implementation.
