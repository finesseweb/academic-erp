...
- Access-management resources satisfy both ownership and creator-hierarchy visibility: University administration may manage only University-origin users/roles; College administration remains restricted to the route College.
...
## Admission Seat Allocation authorization — 2026-09-03
- Page read requires `college_admission_seat_allocation.view` in the exact College scope.
- Physical allocation/re-allocation requires sensitive `college_admission_seat_allocation.allocate`.
- Pre-confirmation cancellation requires sensitive `college_admission_seat_allocation.cancel`.
- Direct URL/API access repeats the same backend `hasCollegePermission(..., college_id)` check; sidebar visibility is not authorization.
- Service-level ownership additionally verifies that the Merit row / Selection Rule / Intake / Reservation context belongs to the same College.

### Admission Document Verification / Seat gate — 2026-09-03
1. All verification routes require authenticated active verified staff plus College-scoped permission.
2. Document download verifies the field value belongs to an Application owned by the route College and that its field type is FILE/IMAGE; bytes stay on private local storage.
3. Review/finalize writes are College-owned and audited.
4. Seat Allocation transaction locks the Application and exact verification row and requires `status=VERIFIED`; frontend button state is not trusted.
