...
- The College list contains only `COLLEGE_STAFF` whose `primary_college_id` equals the route College.
- The current College Administrator is not hidden; if it is a College-owned staff account it appears in the same list.
- The list displays active roles whose assignment scope is the current College.
- Applicant identities are not reachable through College staff lifecycle/role routes.
- Email is trim/lowercase normalized and globally unique across the shared `users` identity table.
