...
## Current Credit Treatment Contract - 2026-09-12

This section supersedes the earlier pre-credit `No Credit field` and `Credit Summary is next` statements.

- Each Curriculum Slot stores `credits` and `credit_counting`.
- `credit_counting` is required for new/edited Slots, accepts `COUNTABLE` or `NON_COUNTABLE`, and defaults to `COUNTABLE` for existing/new records as defined by the schema and UI.
- `COUNTABLE` includes the Slot credit in Required Credits and Maximum Credits.
- `NON_COUNTABLE` retains and displays the numeric credit but excludes it from required, earned, and degree-completion credit totals.
- Credit Treatment belongs to the Slot, not Course Master or Course Mapping. Every mapped Course inherits its Slot treatment.
- Slot Clone and Curriculum Amendment preserve both credit and treatment.
- Curriculum Credit Summary is implemented and excludes non-countable Slots from its totals.
- Credit Treatment does not determine SGPA/CGPA participation; that remains a Result Processing rule.

Governing decision: `DECISIONS/094_CURRICULUM_COURSE_CREDIT_COUNTING.md`.
