...
University Roles lists only roles with `created_by_scope_type = UNIVERSITY`. University-created College-owned roles remain visible and retain their College owner label/filter. College-created roles are excluded from University Role Management and remain available in the owning College role context. Summary counts and direct actions use the same backend scope.
