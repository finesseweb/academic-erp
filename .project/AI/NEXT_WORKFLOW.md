...
## Immediate next QA — ADR 184
Use a real Razorpay TEST credential pair (never LIVE credentials) on one ACTIVE Razorpay TEST profile and run `Create ₹1 Razorpay TEST order`. Confirm an `order_...` provider id is returned by toast. Dummy credentials are expected to fail and are not a valid connectivity QA.

ADR 184 is QA PASS. Before starting ADR 185 fee-linked checkout, complete ADR 187 provider parity QA for Cashfree and PayU so all three retained gateways have verified TEST connectivity.

### Next QA after ADR186
1. Confirm all previously available Test Data Cleanup menus/design are restored.
2. Confirm new `Gateway Test Orders` menu is visible.
3. After an ADR184 Razorpay TEST order exists, clean it and confirm the gateway credential profile/routing remains intact.


## Immediate QA — ADR 187
### Cashfree
1. Use a real Cashfree Sandbox App ID + Secret Key on an ACTIVE CASHFREE / TEST profile.
2. Confirm the flask action appears only while TEST + ACTIVE.
3. Click once and expect `Cashfree TEST order created successfully` with a Cashfree `order_id` and ₹1.00.
4. Confirm the matching row in `online_payment_transactions`: provider CASHFREE, purpose `CREDENTIAL_TEST_ORDER`, amount 1.00, status `ORDER_CREATED`.
5. Confirm the same order exists in Cashfree Sandbox Dashboard.
6. Negative test: deactivate, alter Secret Key/App ID, reactivate, click flask and confirm in-app rejection + persisted FAILED attempt; then restore correct Sandbox credentials.
7. Confirm no flask action on CASHFREE / LIVE.

### PayU
1. Obtain PayU Test Merchant Key + Salt and save them on an ACTIVE PAYU / TEST profile.
2. Confirm the flask action appears only while TEST + ACTIVE.
3. Click once and expect `PayU TEST API authenticated successfully` with the generated PUTEST reference.
4. Confirm `online_payment_transactions`: provider PAYU, purpose `CREDENTIAL_TEST_API`, amount 0.00, status `API_VERIFIED`, provider_status `API_AUTHENTICATED`.
5. Negative test: deactivate, alter Key/Salt, reactivate and confirm the API probe fails in-app and persists FAILED; restore the valid TEST credentials.
6. Confirm no flask action on PAYU / LIVE.

### Webhook foundation
- Cashfree webhook path: `/payments/webhooks/cashfree`.
- PayU webhook path: `/payments/webhooks/payu`.
- Provider dashboards require a publicly reachable HTTPS URL; localhost cannot receive external provider callbacks without a tunnel/staging URL.
- Do not post Fee Payments during webhook QA. Current webhook scope is authenticity verification + provider-transaction reconciliation only.

After Cashfree + PayU TEST QA PASS, update project state and proceed to ADR 185 fee-linked checkout/payment posting through the existing `FeePaymentService`.

## Current provider-gateway QA checkpoint — 2026-09-11
1. Razorpay TEST order/connectivity QA — PASS.
2. Cashfree Sandbox ₹1 order/connectivity QA — PASS.
3. PayU TEST credential/API probe — NEXT.
4. Razorpay/Cashfree/PayU webhook delivery + signature + duplicate/idempotency QA — DEFERRED until a public HTTPS callback/tunnel is available.
5. Only after provider QA is complete should the fee-linked checkout/payment-posting workflow proceed through the existing `FeePaymentService`.

## Immediate QA — ADR 188 Fee-Linked TEST Checkout
Provider connectivity parity is complete: Razorpay PASS, Cashfree PASS, PayU PASS. Next gate is direct fee-linked TEST checkout.

1. Run `php artisan migrate`, `php artisan optimize:clear`, then rebuild frontend.
2. Keep only TEST routing for QA; do not enable LIVE checkout.
3. Start with one Fee Demand whose payable Fee Heads all route to the same ACTIVE Razorpay TEST credential profile.
4. Payment Collection → Collect Payment → select amount/options → `Pay Online (TEST)`.
5. Complete Razorpay TEST payment and verify one normal Fee Payment receipt is created through FeePaymentService; verify allocations and online transaction link/status `PAYMENT_POSTED`.
6. Repeat direct checkout QA with one same-profile Cashfree-routed demand/selection.
7. Repeat direct checkout QA with one same-profile PayU-routed demand/selection.
8. Replay/refresh verification and confirm no duplicate receipt is created.
9. Select an amount spanning Fee Heads routed to two credential profiles; checkout must be blocked before provider order creation.
10. Select an unassigned Fee Head; checkout must be blocked before provider order creation.
11. Re-test offline collection to confirm no regression.
