...
class CollegeAdmissionInterviewController extends Controller {
 public function index(Request $request,College $college,CollegeReservationService $reservationService):Response{
  ...
  $evaluators=User::query()
   ...
   ->with(['roles'=>fn($q)=>$q->select('roles.id','roles.name','roles.code')->where('roles.status','ACTIVE')->wherePivot('status','ACTIVE')->wherePivot('scope_type','COLLEGE')->wherePivot('scope_reference',"college:{$college->id}")->where(fn($r)=>$r->whereNull('user_roles.effective_from')->orWhere('user_roles.effective_from','<=',now()))->where(fn($r)=>$r->whereNull('user_roles.effective_until')->orWhere('user_roles.effective_until','>=',now()))])
   ->orderBy('name')->get(['id','name','email','account_type','primary_college_id']);
