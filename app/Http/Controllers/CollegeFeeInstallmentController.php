...
class CollegeFeeInstallmentController extends Controller
{
    ...
    public function bulkStore(Request $request, College $college): RedirectResponse
    {
        ...
        foreach($selected as $id){
            ...
            foreach($rows->values() as $i=>$row){
                ...
                $allocated+=$amount;
                $amountRows[]=['amount'=>number_format($amount,2,'.',''),'due_date'=>$row['due_date'],'allocation_percentage'=>(float)$row['percentage']];
            }
    ...
    private function bulkItems(College $college,array $data)
    {
        return FeeDemandItem::query()->with([
          ...
          ->whereHas('demand',fn($q)=>$q->where('college_id',$college->id)->where('college_program_offering_id',$data['offering_id'])->where('demand_context',$data['purpose'])->where('billing_basis_group',$data['basis_group'])->where('billing_period_no',$data['period_no'])->where('status','!=','CANCELLED'))
    ...
    private function replaceSchedule(Request $request,FeeDemand $demand,FeeDemandItem $item,$rows,string $mode): void
    {
        DB::transaction(function()use($request,$demand,$item,$rows,$mode){
            $before=FeeInstallmentSchedule::where('fee_demand_item_id',$item->id)->where('status','ACTIVE')->get()->toArray();
            ...
            $rowTotal=max((float)$rows->sum(fn($r)=>(float)$r['amount']),0.01);
            $created=[];foreach($rows->values() as $i=>$row){
                $percentage=array_key_exists('allocation_percentage',$row)?(float)$row['allocation_percentage']:round(((float)$row['amount']/$rowTotal)*100,4);
                $created[]=FeeInstallmentSchedule::create(['fee_demand_id'=>$demand->id,'fee_demand_item_id'=>$item->id,'installment_no'=>$i+1,'amount'=>$row['amount'],'paid_amount'=>0,'allocation_percentage'=>$percentage,'source_mode'=>$mode,'due_date'=>$row['due_date'],'status'=>'ACTIVE','created_by'=>$request->user()->id])->toArray();
            }
