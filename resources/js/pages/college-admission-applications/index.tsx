...
function DynamicInput({field,application,value,values,onChange,copyLocked=false,error}:{field:DynamicField;application?:Application;value:string|string[];values:Record<number,string|string[]>;onChange:(value:string|string[])=>void;copyLocked?:boolean;error?:string}){
    ...
    const label=<Label>{field.label}{field.is_required&&<span className="text-destructive"> *</span>}</Label>;const valuesForValidation:Record<number,unknown>={...values,[field.id]:value};const clientError=(value===undefined||value===null||value===''||(Array.isArray(value)&&value.length===0))?null:admissionFieldValidationError(field,value,valuesForValidation);const displayError=error??clientError??undefined;const validationHint=admissionFieldValidationHint(field);const vr=field.validation_rules??{};const exact=Number(vr.exact_length??0)||undefined;const minLength=exact??(Number(vr.min_length??0)||undefined);const maxLength=exact??(Number(vr.max_length??0)||undefined);const textMode=String(vr.text_input_mode??'ANY');const textInputMode=textMode==='DIGITS_ONLY'?'numeric':undefined;const validateTextInput=(element:HTMLInputElement|HTMLTextAreaElement,nextValue:string)=>element.setCustomValidity(textInputConstraint(textMode,nextValue));
    if(field.field_type==='TEXTAREA')return <div className="space-y-2">{label}<textarea name={base} required={field.is_required} value={String(value??'')} onChange={e=>{validateTextInput(e.currentTarget,e.target.value);onChange(e.target.value)}} onInvalid={e=>validateTextInput(e.currentTarget,String(value??''))} disabled={copyLocked} minLength={minLength} maxLength={maxLength} className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder={field.placeholder??''}/>{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}{validationHint&&<p className="text-[11px] text-muted-foreground">{validationHint}</p>}{displayError&&<p className="text-xs text-destructive">{displayError}</p>}</div>;
    if(field.field_type==='SELECT')return <div className="space-y-2">{label}<select name={base} required={field.is_required} value={String(value??'')} disabled={copyLocked} onChange={e=>onChange(e.target.value)} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="">Select...</option>{field.options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select>{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}{validationHint&&<p className="text-[11px] text-muted-foreground">{validationHint}</p>}{displayError&&<p className="text-xs text-destructive">{displayError}</p>}</div>;
    if(field.field_type==='RADIO'||field.field_type==='YES_NO')return <div className="space-y-2">{label}<div className="flex flex-wrap gap-4">{field.options.map(o=><label key={o.value} className="flex items-center gap-2 text-sm"><input type="radio" name={base} value={o.value} required={field.is_required} disabled={copyLocked} checked={String(value??'')===o.value} onChange={()=>onChange(o.value)} className="size-4"/>{o.label}</label>)}</div>{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}{validationHint&&<p className="text-[11px] text-muted-foreground">{validationHint}</p>}{displayError&&<p className="text-xs text-destructive">{displayError}</p>}</div>;
    ...
    if(field.field_type==='FILE'||field.field_type==='IMAGE')return <div className="space-y-2">{label}<Input name={base} type="file" required={field.is_required&&!existing?.file_name} accept={field.field_type==='IMAGE'?'image/*':undefined}/>{existing?.file_name&&<p className="text-xs text-muted-foreground">Existing: {existing.file_name}. Upload a new file only to replace it.</p>}{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}{validationHint&&<p className="text-[11px] text-muted-foreground">{validationHint}</p>}{displayError&&<p className="text-xs text-destructive">{displayError}</p>}</div>;
    if(field.field_type==='DATE')return <div className="space-y-2">{label}<DatePicker id={`dynamic-field-${field.id}`} name={base} value={String(value??'')} onValueChange={onChange}/>{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}{validationHint&&<p className="text-[11px] text-muted-foreground">{validationHint}</p>}{displayError&&<p className="text-xs text-destructive">{displayError}</p>}</div>;
    const inputType=field.field_type==='NUMBER'?'number':field.field_type==='EMAIL'?'email':field.field_type==='PHONE'?'tel':'text';
    return <div className="space-y-2">{label}<Input name={base} type={inputType} required={field.is_required} value={String(value??'')} disabled={copyLocked} minLength={minLength} maxLength={maxLength} min={field.field_type==='NUMBER'&&vr.min_value!==undefined?Number(vr.min_value):undefined} max={field.field_type==='NUMBER'&&vr.max_value!==undefined?Number(vr.max_value):undefined} step={field.field_type==='NUMBER'?(vr.integer_only?'1':vr.decimal_places!==undefined?String(1/Math.pow(10,Number(vr.decimal_places))):'any'):undefined} inputMode={field.field_type==='TEXT'?textInputMode:undefined} onChange={e=>{if(field.field_type==='TEXT')validateTextInput(e.currentTarget,e.target.value);onChange(e.target.value)}} onInvalid={e=>{if(field.field_type==='TEXT')validateTextInput(e.currentTarget,String(value??''))}} placeholder={field.placeholder??''}/>{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}{validationHint&&<p className="text-[11px] text-muted-foreground">{validationHint}</p>}{displayError&&<p className="text-xs text-destructive">{displayError}</p>}</div>;
}
