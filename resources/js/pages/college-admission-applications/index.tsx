...
function DynamicInput({field,application,value,onChange}:{field:DynamicField;application?:Application;value:string|string[];onChange:(value:string|string[])=>void}){
    ...
    return <div className="space-y-2">{label}<Input name={base} type={inputType} value={String(value??'')} onChange={e=>onChange(e.target.value)} placeholder={field.placeholder??''}/>{field.help_text&&<p className="text-xs text-muted-foreground">{field.help_text}</p>}</div>;
}
                ...
                {({processing,errors})=><>
                    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-1 py-2 pr-2">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2"><Label>Admission Mode</Label><select name="admission_mode" value={admissionMode} onChange={e=>setAdmissionMode(e.target.value as 'REGULAR'|'DIRECT')} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="REGULAR">Regular / Selection Based</option><option value="DIRECT">Direct Admission</option></select><p className="text-xs text-muted-foreground">Application capture is common. Selection / Merit / Reservation / Seat Allocation remain downstream processes.</p></div>
