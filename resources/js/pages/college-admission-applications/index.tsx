                ...
                {({processing,errors})=><>
                    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-1 py-2 pr-2">
                        ...
                        <div className="grid min-w-0 gap-4 sm:grid-cols-2">
                            ...
                            <div className="min-w-0 space-y-2"><Label>Date of Birth</Label><Input className="w-full min-w-0" type="date" name="date_of_birth" defaultValue={application?.date_of_birth?.slice(0,10)??''}/>{errors.date_of_birth&&<p className="text-xs text-destructive">{errors.date_of_birth}</p>}</div>
...
function ConfirmAction({label,title,description,action,method='patch',icon}:{label:string;title:string;description:string;action:string;method?:'patch'|'post';icon:'submit'|'withdraw'}){
    return <Dialog><DialogTrigger asChild><Button size="sm" variant="outline">{icon==='submit'?<Send/>:<XCircle/>}{label}</Button></DialogTrigger><DialogContent className="w-[calc(100vw-2rem)] sm:!max-w-lg"><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{description}</DialogDescription></DialogHeader><Form action={action} method={method}>{({processing})=><DialogFooter><DialogClose asChild><Button type="button" variant="outline">Cancel</Button></DialogClose><Button type="submit" variant="outline" disabled={processing}>{processing&&<Spinner/>}{icon==='submit'?<Send/>:<XCircle/>}{label}</Button></DialogFooter>}</Form></DialogContent></Dialog>;
}
