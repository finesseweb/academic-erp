...
type Policy = {
    id: number; name: string; code: string; version: string; scope_type: 'UNIVERSITY'|'PROGRAM_TEMPLATE'|'CURRICULUM';
    ...
    description: string|null; academic_session: Option; program_template?: Option|null; curriculum?: {id:number;name:string;code:string;version:string}|null;
};
type Props = {
    policies: { data: Policy[]; links: {url:string|null;label:string;active:boolean}[]; total:number };
    filters: {search?:string;status?:string}; academicSessions: Option[]; programTemplates: Option[]; curricula: CurriculumOption[];
...
const empty = { academic_session_id:'', program_template_id:'', curriculum_id:'', name:'', code:'', version:'1.0', scope_type:'UNIVERSITY', effective_from:'', effective_to:'', description:'' };

export default function AcademicPolicyIndex({policies, filters, academicSessions, programTemplates, curricula, approvalWorkflows, permissions}:Props) {
    ...
    const openCreate=()=>{ setEditing(null); form.setData(empty); form.clearErrors(); setShowForm(true); };
    const openEdit=(p:Policy)=>{ setEditing(p); form.setData({academic_session_id:String(p.academic_session.id),program_template_id:p.program_template?String(p.program_template.id):'',curriculum_id:p.curriculum?String(p.curriculum.id):'',name:p.name,code:p.code,version:p.version,scope_type:p.scope_type,effective_from:p.effective_from??'',effective_to:p.effective_to??'',description:p.description??''}); form.clearErrors(); setShowForm(true); };
    ...
    return <>
        ...
        {showForm&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 p-4 backdrop-blur-sm"><div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl border bg-card shadow-xl"><div className="flex items-start justify-between border-b p-5"><div><h2 className="text-lg font-semibold">{title}</h2><p className="mt-1 text-sm text-muted-foreground">Phase 1 policy header. Approval and amendment will use this same versioned record.</p></div><Button variant="ghost" size="icon" onClick={()=>setShowForm(false)}><X className="size-4"/></Button></div><form onSubmit={submit} className="grid gap-4 p-5 md:grid-cols-2">
            ...
            <Field label="Academic Session" error={form.errors.academic_session_id}><Select value={form.data.academic_session_id} onValueChange={v=>{form.setData('academic_session_id',v);form.setData('curriculum_id','')}}><SelectTrigger><SelectValue placeholder="Select Session"/></SelectTrigger><SelectContent>{academicSessions.map(o=><SelectItem key={o.id} value={String(o.id)}>{o.name} ({o.code})</SelectItem>)}</SelectContent></Select></Field>
            ...
            <Field label="Scope" error={form.errors.scope_type}><Select value={form.data.scope_type} onValueChange={v=>{form.setData('scope_type',v); if(v==='UNIVERSITY'){form.setData('program_template_id','');form.setData('curriculum_id','')} if(v==='PROGRAM_TEMPLATE')form.setData('curriculum_id','')}}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="UNIVERSITY">University-wide</SelectItem><SelectItem value="PROGRAM_TEMPLATE">Program Template</SelectItem><SelectItem value="CURRICULUM">Curriculum</SelectItem></SelectContent></Select></Field>
            {form.data.scope_type!=='UNIVERSITY'&&<Field label="Program Template" error={form.errors.program_template_id}><Select value={form.data.program_template_id} onValueChange={v=>{form.setData('program_template_id',v);form.setData('curriculum_id','')}}><SelectTrigger><SelectValue placeholder="Select Program"/></SelectTrigger><SelectContent>{programTemplates.map(o=><SelectItem key={o.id} value={String(o.id)}>{o.name} ({o.code})</SelectItem>)}</SelectContent></Select></Field>}
