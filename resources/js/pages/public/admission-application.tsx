...
type Field={id:number;field_key:string;label:string;field_type:string;placeholder?:string|null;help_text?:string|null;is_required:boolean;college_admission_form_panel_id?:number|null;options?:Option[];conditions?:Condition[];condition_match_mode?:'ALL'|'ANY';validation_rules?:Record<string,unknown>|null;comparison_rule?:{source_field_id:number;source_field_label?:string|null;operator:string}|null;copy_rule?:{source_field_id:number;source_field_label?:string|null;trigger_field_id:number;trigger_field_label?:string|null;trigger_values:string[];read_only:boolean}|null};
...
function DynamicField({field,values,setValue,errors}:{field:Field;values:Record<number,unknown>;setValue:(id:number,value:unknown)=>void;errors:Record<string,string>}){
    ...
    if(field.field_type==='TEXTAREA')return <div className="space-y-2">{common}<textarea id={`field-${field.id}`} name={name} required={field.is_required} disabled={copyLocked} minLength={minLength} maxLength={maxLength} placeholder={field.placeholder??''} value={String(values[field.id]??'')} onChange={e=>{validateTextInput(e.currentTarget,e.target.value);setValue(field.id,e.target.value)}} onInvalid={e=>validateTextInput(e.currentTarget,String(values[field.id]??''))} className="min-h-28 w-full rounded-xl border bg-background px-3 py-2 text-sm outline-none focus:border-primary/60 focus:ring-[3px] focus:ring-primary/10"/>{error&&<p className="text-xs text-destructive">{error}</p>}</div>;
...
function PublicAdmissionApplication({publicForm,applicant,successApplicationNo}:Props){
    ...
    const validatePage=(pageIndex:number)=>{if(!stepByStep)return true;const page=document.querySelector(`[data-public-form-page="${pageIndex}"]`);if(!page)return true;if(pageIndex>=1&&pageIndex<=steps.length){for(const field of safe(steps[pageIndex-1]?.fields)){const rule=field.comparison_rule;if(!rule)continue;const left=values[field.id],right=values[rule.source_field_id];if(left===undefined||left===''||right===undefined||right==='')continue;const a=field.field_type==='DATE'?new Date(String(left)).getTime():Number(left);const b=field.field_type==='DATE'?new Date(String(right)).getTime():Number(right);const ok=rule.operator==='LT'?a<b:rule.operator==='LTE'?a<=b:rule.operator==='GT'?a>b:rule.operator==='GTE'?a>=b:rule.operator==='EQ'?a===b:a!==b;if(!ok){const control=page.querySelector(`[name="custom_fields[${field.id}]"]`) as HTMLInputElement|null;if(control){const symbol=({LT:'<',LTE:'≤',GT:'>',GTE:'≥',EQ:'=',NEQ:'≠'} as Record<string,string>)[rule.operator]??rule.operator;control.setCustomValidity(`${field.label} must be ${symbol} ${rule.source_field_label??'the selected field'}.`);control.reportValidity();setTimeout(()=>control.setCustomValidity(''),0);}return false;}}}for(const control of Array.from(page.querySelectorAll('input, select, textarea')) as Array<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>){if(!control.checkValidity()){control.reportValidity();return false;}}return true;};
