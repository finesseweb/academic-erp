...
type CandidateCategory={id:number|null;code:string;name:string;source:'SYSTEM_FORM_FIELD'|'LEGACY_FORM_MAPPING'|'MANUAL'};
type Allocation = {
...
type Row = {
    merit_entry_id?:number|null; admission_mode?:'REGULAR'|'DIRECT'; rank:number|null; final_weighted_score:number|null; application_id:number; choice_id:number;
    program_name?:string|null; session_name?:string|null; bucket_key?:string; capacity?:Capacity;
...
type DirectScreen = { candidate_category_options:{id:number;name:string;code:string}[]; summary:{direct_candidates:number;verified:number;allocated_count:number}; rows:Row[] };
type Props = { college:{id:number;name:string;code:string;status:string}; rules:Rule[]; selectedRuleId:number|null; screen:Screen; directScreen:DirectScreen; can:{allocate:boolean;cancel:boolean} };

function AllocationDialog({collegeId,row,capacity,candidateCategoryOptions,direct=false}:{collegeId:number;row:Row;capacity:Capacity;candidateCategoryOptions:{id:number;name:string;code:string}[];direct?:boolean}) {
    const [open,setOpen]=useState(false);
    const mappedCategory=['SYSTEM_FORM_FIELD','LEGACY_FORM_MAPPING'].includes(row.candidate_reservation_category?.source??'')?row.candidate_reservation_category:null;
    ...
    const submit=()=>{
        setProcessing(true);
        router.post(direct?`/college/${collegeId}/admission-seat-allocations/direct/${row.choice_id}`:`/college/${collegeId}/admission-seat-allocations/merit/${row.merit_entry_id}`,{
...
export default function CollegeAdmissionSeatAllocations({college,rules,selectedRuleId,screen,directScreen,can}:Props) {
    ...
    return <><Head title="Seat Allocation / Consumption"/><div className="space-y-6 p-4 md:p-6">
        ...
        {directScreen.rows.length>0&&<Card><CardHeader><CardTitle>Direct Admission → Seat Decisions</CardTitle></CardHeader><CardContent className="space-y-3"><div className="rounded-md border bg-muted/20 p-3 text-sm text-muted-foreground">Direct Admission bypasses Eligibility / Score / Interview / Merit only. VERIFIED documents, Intake capacity, candidate reservation category and physical seat consumption are still enforced.</div><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-muted/30 text-left"><tr><th className="px-3 py-2">Candidate</th><th className="px-3 py-2">Application</th><th className="px-3 py-2">Programme / Session</th><th className="px-3 py-2">Discipline</th><th className="px-3 py-2">Documents</th><th className="px-3 py-2">Capacity</th><th className="px-3 py-2">Seat Decision</th><th className="px-3 py-2">Actions</th></tr></thead><tbody>{directScreen.rows.map(row=>{const capacity=row.capacity!;return <tr key={row.choice_id} className="border-t align-top"><td className="px-3 py-3"><div className="font-medium">{row.candidate_name}</div><div className="text-xs text-muted-foreground">DIRECT ADMISSION</div></td><td className="px-3 py-3">{row.application_no}</td><td className="px-3 py-3"><div>{row.program_name??'Program'}</div><div className="text-xs text-muted-foreground">{row.session_name??'Session'} · {row.bucket_key}</div></td><td className="px-3 py-3">{row.discipline_name??'—'}{row.specialization_name?` → ${row.specialization_name}`:''}</td><td className="px-3 py-3">{row.document_verification?.status==='VERIFIED'?<span className="font-medium text-emerald-700">VERIFIED</span>:<span className="font-medium text-amber-700">{row.document_verification?.status??'Pending'}</span>}</td><td className="px-3 py-3"><div>{capacity.total_used}/{capacity.basis_capacity} used</div><div className="text-xs text-muted-foreground">Open {capacity.open.remaining} remaining</div></td><td className="px-3 py-3">{!row.allocation?<span className="text-muted-foreground">Not allocated</span>:row.allocation.status==='ALLOCATED'?<div><div className="flex items-center gap-1 font-medium text-emerald-700"><CheckCircle2 className="size-4"/>{row.allocation.physical_category_name??'Open / Unreserved'}</div><div className="text-xs text-muted-foreground">Round {row.allocation.allocation_round}</div></div>:<div><div className="font-medium text-destructive">Cancelled</div><div className="text-xs text-muted-foreground">{row.allocation.cancellation_reason}</div></div>}</td><td className="px-3 py-3"><div className="flex flex-wrap gap-2">{can.allocate&&row.document_verification?.status==='VERIFIED'&&(!row.allocation||row.allocation.status==='CANCELLED')&&<AllocationDialog collegeId={college.id} row={row} capacity={capacity} candidateCategoryOptions={directScreen.candidate_category_options} direct/>}{can.cancel&&row.allocation?.status==='ALLOCATED'&&<CancelButton collegeId={college.id} row={row}/>}</div></td></tr>})}</tbody></table></div></CardContent></Card>}
