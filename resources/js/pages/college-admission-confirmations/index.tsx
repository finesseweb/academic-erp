...
type Row = {
    ...
    preference_no:number; merit_rank:number; final_weighted_score:number; document_verification_status:string|null;
...
function ConfirmDialog({collegeId,row}:{collegeId:number;row:Row}) {
    ...
    return <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
            <DialogHeader><DialogTitle>Confirm Admission · {row.application_no}</DialogTitle><DialogDescription>This confirms the candidate against Seat Allocation #{row.seat_allocation_id}. Capacity, reservation, merit rank and document verification will not be recalculated.</DialogDescription></DialogHeader>
            <div className="space-y-3 py-2"><div className="rounded-md border p-3 text-sm"><div className="font-medium">{row.candidate_name} · Merit Rank #{row.merit_rank}</div><div className="mt-1 text-muted-foreground">{row.rule?.program_name??'Program'} · {row.rule?.session_name??'Session'} · {row.physical_category_name??'Open / Unreserved'} · Round {row.allocation_round}</div></div><div className="space-y-2"><Label>Approval / confirmation note <span className="text-muted-foreground">(optional)</span></Label><Textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Record counselling/committee reference or other approval note where required."/></div></div>
